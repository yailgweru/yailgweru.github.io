...
under the `file://` origin. Serving the repo root with any static file
server is still the correct way to develop/preview the site normally:
...
the same way. Over `http://`/`https://`, `data.json` and
`blogs/manifest.json` are always fetched live — editing `data.json` and
reloading is enough, same as before.

**Double-clicking `index.html` (`file://`) also works**, as a fallback
for quick local checks with no server at all — but it's a degraded mode,
not the primary workflow:

- `fetch('data.json')` fails under `file://`, so `index.html` falls back
  to an inline copy embedded in a generated `<script id="site-data">`
  block (marked `INLINE-DATA:START`/`END`). That block is stale the
  moment `data.json` changes — run `python scripts/inline_data.py` to
  resync it before checking a `data.json` edit under `file://`. Under a
  real server this fallback is never used.
- `blogs/manifest.json` and per-post HTML fetches have no such fallback
  and just fail gracefully under `file://` (the Blogs section renders
  empty, read-more panels that use `more.fetch` show their
  `more.fallback` text) — this is expected, not a bug.
- The 3D scene's center logo disc is a WebGL texture, which is a
  stricter case than a plain `fetch()`: Chrome treats every `file://`
  path as its own unique origin, so WebGL refuses to texture an
  `<img>` loaded from `assets/logo.png` there (`SecurityError` /
  "cross-origin data"). It's loaded from a generated inline base64
  `data:` URI instead (`INLINE-LOGO:START`/`END` block) so it works in
  both modes — resync with `python scripts/inline_logo.py` after
  `logo.png` changes. The header `<img>` logo isn't affected (plain
  `<img>` display doesn't hit this WebGL restriction) and keeps using
...
site) — those paths don't read the inline blocks at all.
...
scripts/
  ...
  prerender_seo.py               # committed — bakes SEO head/JSON-LD/#seo-content/blog cards into static HTML
...
- **SEO-critical HTML is prerendered into the static files** by
  `python scripts/prerender_seo.py` (committed), which reads `data.json` +
  `blogs/manifest.json` and rewrites four generated marker blocks:
  `INLINE-SEO-HEAD` (title/meta/OG/Twitter/canonical) and `INLINE-LD-JSON`
  (Organization + WebSite `@graph`) in `index.html`'s head,
  `INLINE-SEO-CONTENT` (the full `#seo-content` semantic mirror of every
  section, including blog links) in `index.html`'s body, and
...
- Don't hand-edit the `INLINE-DATA` or `INLINE-LOGO` blocks in
  `index.html` — they're generated by `scripts/inline_data.py` /
  `scripts/inline_logo.py` from `data.json` / `assets/logo.png`. If you
  change either source and care about the `file://` fallback path (see
  "What this is"), rerun the matching script and commit the result.
- Likewise don't hand-edit the `INLINE-SEO-HEAD`, `INLINE-LD-JSON`,
  `INLINE-SEO-CONTENT` (in `index.html`) or `INLINE-BLOG-LIST` (in
  `blogs/index.html`) blocks — rerun `python scripts/prerender_seo.py`
  after editing `data.json` or publishing/removing a post, and commit the
  result (see "SEO").
